...
class ResetPasswordController extends Controller
{
    public function showForgotPassword(Request $request)
    {
        ...
        if (!$validator->fails()) {
            return response()->view('cms.auth.forgot-password', ['guard' => $request->input('guard')]);
        } else {
    ...
    public function emailForgetPassword(Request $request)
    {
        $validator = Validator($request->all(), [
            ...
            'guard' => 'required|in:admin,user,profession|string',
        ]);
        if (!$validator->fails()) {
            if ($request->get('guard') == 'admin') {
                ...
                $broker = 'users';
            }elseif($request->get('guard') == 'profession') {
                $email =  Profession::where('email', $request->get('email'))->first();
                $broker = 'professions';
            }
            if (!is_null($email)) {
