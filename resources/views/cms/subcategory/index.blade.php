...
<div class="content-wrapper">
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            ...
            <div class="col-12">
                <h2 class="content-header-title float-left mb-0">{{__('cms.SubCategories')}}</h2>
                ...
                <table class="table data-thumb-view dataTable no-footer" id="DataTables_Table_0" role="grid">
                    ...
                    <tbody>
                        ...
                        <tr role="row" class="odd">
                            <td>{{$loop->index +1}}</td>
                            <td class="product-name">{{$subcategory->name_ar}}</td>
                            <td class="product-category">{{$subcategory->name_en}}</td>
                            <td>
                                <div>
                                    <div class="badge @if ($subcategory->status) bg-success @else  bg-danger
                                    @endif">{{$subcategory->status}}</div>
                                </div>
                            </td>
                            <td>
                                @if (app()->getLocale()=='ar')
                            ...
                            <td style="width: 166.9062px;">
                                <a href="{{route('subcategories.edit', $subcategory->id)}}" class="btn btn-info btn-sm-flat">
                                    <i class="feather icon-edit" style="font-size: 18px;"></i></a>
                                <a href="#" onclick="confirmDelete('{{$subcategory->id}}', this)" class="btn btn-danger btn-sm-flat">
